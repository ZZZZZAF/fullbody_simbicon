...
class MyWorldNode(dart.gui.osg.RealTimeWorldNode):
    def __init__(self, world, skel):
        ...
        self.left_heel_ry = self.skel.getDof('j_heel_left_ry').getIndexInSkeleton()
        self.left_heel_rz = self.skel.getDof('j_heel_left_rz').getIndexInSkeleton()
        ...
        self.right_heel_ry = self.skel.getDof('j_heel_right_ry').getIndexInSkeleton()
        self.right_heel_rz = self.skel.getDof('j_heel_right_rz').getIndexInSkeleton()
        ...
        self.state0[self.left_thigh_ry] = -0.2
        self.state0[self.left_shin] = 0.35
        self.state0[self.left_heel_ry] = -0.2
        self.state0[self.right_thigh_ry] = -0.2
        self.state0[self.right_shin] = 0.35
        self.state0[self.right_heel_ry] = -0.2

        temp = self.skel.getPositions()
        temp[self.torso_ry] = 0.0
        temp[self.left_thigh_ry] = -0.5
        temp[self.left_shin] = 1.1
        temp[self.left_heel_ry] = -0.6
        temp[self.left_bicep] = 0.3
        temp[self.left_forearm] = -0.2
        temp[self.right_shin] = 0.05
        temp[self.right_heel_ry] = 0.0
        ...
        temp[self.right_heel_ry] = -0.6
        temp[self.right_bicep] = 0.3
        temp[self.right_forearm] = -0.2
        temp[self.left_shin] = 0.05
        temp[self.left_heel_ry] = 0.0
        temp[self.left_bicep] = -0.3
        temp[self.left_forearm] = -0.2
        self.state3 = temp
        ...
        for i in range(6, self.dofs):
            self.Kp_stand[i, i] = 500.0
            self.Kd_stand[i, i] = 3

            self.Kp[i, i] = 240.0
            self.Kd[i, i] = 3

        self.Kp[self.left_shin, self.left_shin] = 500.0
        self.Kd[self.left_shin, self.left_shin] = 50.0

        self.Kp[self.right_shin, self.right_shin] = 400.0
        self.Kd[self.right_shin, self.right_shin] = 40.0

        self.Kp[self.left_heel_rz, self.left_heel_rz] = 250.0
        self.Kd[self.left_heel_rz, self.left_heel_rz] = 10.0

        self.Kp[self.left_heel_ry, self.left_heel_ry] = 200.0
        self.Kd[self.left_heel_ry, self.left_heel_ry] = 20.0

        self.Kp[self.right_heel_rz, self.right_heel_rz] = 250.0
        self.Kd[self.right_heel_rz, self.right_heel_rz] = 10.0

        self.Kp[self.right_heel_ry, self.right_heel_ry] = 200.0
        self.Kd[self.right_heel_ry, self.right_heel_ry] = 20.0

        self.Kp[self.torso_rx, self.torso_rx] = 1000.0
        self.Kd[self.torso_rx, self.torso_rx] = 100.0

        self.Kp[self.torso_ry, self.torso_ry] = 1000.0
        self.Kd[self.torso_ry, self.torso_ry] = 100.0


        self.Kp_torso_sagital = 800
        self.Kd_torso_sagital = 5000

        self.Kp_torso_coronal = 800
        self.Kd_torso_coronal = 5000

        self.Kp_thigh_sagital =500
        self.Kd_thigh_sagital =50

        self.Kp_thigh_coronal =250
        self.Kd_thigh_coronal =25
    ...
    def customPreStep(self):
        ...
        if self.current_controller == 'stand':
            ...
            if self.elapsed_time >= self.standing_time:
            ...
            if self.current_state == 'state1':
                ...
                # print("self.pelvis.getSpatialVelocity()", self.pelvis.getSpatialVelocity())
                ...
                if constraint_forces[self.left_heel_ry] != 0 :
                ...
                if constraint_forces[self.right_heel_ry] != 0 :
                    self.current_state = 'state1'
    ...
    def setCameraPositionBehind(self):
        ...
        eyePosition = comFrame.multiply([-5,0,0.5])
    ...
    def setCameraPositionLeft(self):
        comFrame = self.getCOMFrame()
        eyePosition = comFrame.multiply([0,5,0.5])
        targetPoint = comFrame.translation()
        self.viewer.setCameraHomePosition(eyePosition,
                                          targetPoint,
                                          [0,0,1])
